 ...
 *
 * Ownership (same gate as the aave-v3 handler): every decoded
...
export class MorphoHandler implements Handler {
  ...
  decode(raw: RawTx, ctx: DecodeContext): DecodeResult {
    ...
    const transfers = erc20Transfers(logs.map(parseLog));
  ...
  private decodeBlueLog(
    ...
    const assetIn = (amount: bigint): string =>
      this.assetSymbol(
        nearestTransfer(transfers, logIndex, (t) => t.to === MORPHO_BLUE_BASE && t.value === amount)
          ?.token ?? marketId,
      );
    const assetOut = (amount: bigint): string =>
      this.assetSymbol(
        nearestTransfer(
          transfers,
          logIndex,
          (t) => t.from === MORPHO_BLUE_BASE && t.value === amount,
        )?.token ?? marketId,
      );
  ...
  private decodeVaultLog(
    ...
    const underlying = (amount: bigint, direction: 'in' | 'out'): string | undefined => {
      const direct = nearestTransfer(
        ...
        logIndex,
        direction === 'in'
          ? (t) => t.to === vault && t.token !== vault && t.value === amount
          : (t) => t.from === vault && t.token !== vault && t.value === amount,
      );
...
function nearestTransfer(
  transfers: readonly Erc20Transfer[],
  logIndex: number,
  match: (t: Erc20Transfer) => boolean,
): Erc20Transfer | undefined {
  let best: Erc20Transfer | undefined;
